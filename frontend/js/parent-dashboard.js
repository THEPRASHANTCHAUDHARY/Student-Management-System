...
function loadChildData() {
    ...
    setTimeout(() => {
        animateCounter('overallAttendance', childData.attendance + '%');
        animateCounter('currentGPA', childData.gpa);
        animateCounter('totalSubjects', childData.subjects);
        animateCounter('classRank', childData.rank + 'th');
    }, 500);
}
...
function animateCounter(elementId, targetValue) {
    ...
    const isOrdinal = targetValue.toString().includes('th');
...
function setupEventListeners() {
    ...
    document.querySelectorAll('.teacher-contact').forEach(contact => {
        contact.addEventListener('click', function() {
            ...
            if (phone) {
                showMessage('📞 Contact information ready to call!', 'success');
            }
        });
    });
}
